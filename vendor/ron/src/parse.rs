...
impl<'a> Bytes<'a> {
    ...
    pub fn any_num(&mut self) -> Result<AnyNum> {
        ...
        if is_float {
            ...
            let max_u8 = LargeUInt::from(u8::MAX);
            let max_u16 = LargeUInt::from(u16::MAX);
            let max_u32 = LargeUInt::from(u32::MAX);
            #[cfg_attr(not(feature = "integer128"), allow(clippy::useless_conversion))]
            let max_u64 = LargeUInt::from(u64::MAX);

            let min_i8 = LargeSInt::from(i8::MIN);
            let max_i8 = LargeSInt::from(i8::MAX);
            let min_i16 = LargeSInt::from(i16::MIN);
            let max_i16 = LargeSInt::from(i16::MAX);
            let min_i32 = LargeSInt::from(i32::MIN);
            let max_i32 = LargeSInt::from(i32::MAX);
            #[cfg_attr(not(feature = "integer128"), allow(clippy::useless_conversion))]
            let min_i64 = LargeSInt::from(i64::MIN);
            #[cfg_attr(not(feature = "integer128"), allow(clippy::useless_conversion))]
            let max_i64 = LargeSInt::from(i64::MAX);
    ...
    pub fn consume_struct_name(&mut self, ident: &'static str) -> Result<bool> {
        ...
        let found_ident = match self.identifier() {
            Ok(maybe_ident) => core::str::from_utf8(maybe_ident)?,
    ...
    pub fn identifier(&mut self) -> Result<&'a [u8]> {
        ...
        if !is_ident_first_char(next) {
            if is_ident_raw_char(next) {
                ...
                if let Ok(ident) = core::str::from_utf8(ident_bytes) {
            ...
            match self.bytes.get(1).ok_or(Error::Eof)? {
                ...
                _ => {
                    ...
                    if raw_ident_length > std_ident_length {
                        if let Ok(ident) = core::str::from_utf8(&self.bytes[..raw_ident_length]) {
                            return Err(Error::SuggestRawIdentifier(String::from(ident)));
                        }
                    }
