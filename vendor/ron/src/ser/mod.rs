...
impl<'a, W: io::Write> ser::Serializer for &'a mut Serializer<W> {
    ...
    fn serialize_f32(self, v: f32) -> Result<()> {
        write!(self.output, "{}", v)?;
        // f32::fract is std-only; on no_std targets compute via libm's trunc.
        #[cfg(target_os = "none")]
        let is_whole = v - libm::truncf(v) == 0.0;
        #[cfg(not(target_os = "none"))]
        let is_whole = v.fract() == 0.0;
        if is_whole {
            write!(self.output, ".0")?;
        }
        Ok(())
    ...
    fn serialize_f64(self, v: f64) -> Result<()> {
        write!(self.output, "{}", v)?;
        #[cfg(target_os = "none")]
        let is_whole = v - libm::trunc(v) == 0.0;
        #[cfg(not(target_os = "none"))]
        let is_whole = v.fract() == 0.0;
        if is_whole {
            write!(self.output, ".0")?;
