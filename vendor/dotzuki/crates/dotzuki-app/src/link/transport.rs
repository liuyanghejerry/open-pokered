...
//! protocol is one message `M` (the game's link wire type) serialized with
//! `serde_json` per line, the same JSON-line convention as the debug server.
//! A background reader thread parses incoming lines and forwards the
//! messages into an `mpsc` channel, so [`TcpTransport::try_recv`] never
//! blocks the game loop — mirroring the in-memory
//! [`ChannelTransport`](dotzuki_engine::link::ChannelTransport) used by
//! engine-side tests.
//!
//! The framing itself ([`encode_line`]/[`decode_line`]) comes from
//! [`dotzuki_engine::link::codec`], shared with every other JSON-line
//! transport (native or wasm) so the wire format stays byte-identical.
...
/// [`ChannelTransport`](dotzuki_engine::link::ChannelTransport)'s semantics
/// exactly: `Ok(None)` when nothing has arrived, `Err(Disconnected)` once
/// the peer is gone.
///
...
fn read_loop<M: DeserializeOwned>(stream: TcpStream, tx: mpsc::Sender<M>) {
...
/// Listener side of link play (the hosting game).
///
/// One peer per game — classic JRPG link play is single-link — so after
...
pub struct LinkServer<M> {
    listener: TcpListener,
    _marker: std::marker::PhantomData<fn() -> M>,
}

impl<M: Serialize + DeserializeOwned + Send + 'static> LinkServer<M> {
    ...
    pub fn accept(&self) -> Result<Option<TcpTransport<M>>, TransportError> {
...
mod tests {
    ...
    /// A stand-in wire protocol with an asymmetric handshake pair plus one
    /// plain message, mirroring the shape of a real link protocol enum.
    ...
    fn tcp_pair() -> (TcpTransport<TestMessage>, TcpTransport<TestMessage>) {
    ...
    fn connect_listen_exchange_hello_ack() {
        ...
        let server: LinkServer<TestMessage> =
            LinkServer::new("127.0.0.1:0".parse().unwrap()).unwrap();
        ...
        assert!(wait_until(Duration::from_secs(5), || {
            matches!(
                ...
                Ok(Some(TestMessage::Hello {
                    version: PROTOCOL_VERSION
                }))
            )
        }));
        server_transport.send(hello_ack()).unwrap();
        assert!(wait_until(Duration::from_secs(5), || {
            matches!(
                client.try_recv(),
                Ok(Some(TestMessage::HelloAck {
                    version: PROTOCOL_VERSION
                }))
            )
        }));
    ...
    fn fragmented_write_is_reassembled() {
        ...
        let mut server = TcpTransport::<TestMessage>::from_stream(raw_server).unwrap();
    ...
    fn send_after_peer_drop_is_disconnected() {
        ...
        drop(server);
        assert!(wait_until(Duration::from_secs(5), || {
            matches!(client.send(hello()), Err(TransportError::Disconnected))
        }));
    }
}
