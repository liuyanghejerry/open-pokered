...
| ID | 场景与固定起点 | 触发与停止条件 | 原版重点检查 |
| --- | --- | --- | --- |
| `battle-entry` | 官方 DEBUG 战斗入口，或固定野生战斗 | 触发战斗；停止在 `Wild ... appeared!` 或正式战斗菜单出现 | 淡入/闪白、双方精灵出现、入场滑动、出现文字；不同队伍时只比较进场层，不比较精灵差异 |
| `fly-departure-arrival` | `PalletTown (5,6)` → `ViridianCity (23,26)` | 在 Town Map 确认目的地；分别记录离场和到达，至玩家稳定落地 | 离场：原地拍翅、两段飞行坐标、白屏；到达：飞鸟从右上方进入并落地，拍翅节拍 |
| `surf-entry` | `PalletTown (5,13)`，面向前方水面（通常为水 tile `0x14`） | 使用 SURF；停止在上水文字完成并可移动 | 确认文字、玩家推进到水面、冲浪精灵/运输状态、后续水面移动 |
| `cut-tree` | `VermilionCity (15,17)`，面向有效 CUT 树（前方 tile `0x3d`） | 使用 CUT；停止在砍树文字完成、地图稳定 | 白屏/重载时序、树块替换、CUT OAM/覆盖层、音效与文字顺序 |
| `ledge-down` | `Route1 (10,4)`，面向下方台阶/ledge | 按住下方向直到 `(10,6)`；单独截取第一跳 | 16 项 `PlayerJumpingYScreenCoords` 弧线、移动方向、阴影、输入锁定和落点 |
