...
| `get_state` | Screen, map, player pos/facing, `active_script_effect` (label), full `script_effect` payload, `dialogue_state` (page/total/char progress/waiting-for-input), `choice` (menu options+cursor), `script_running`, `script_awaiting_battle`, `player_movement_state`, `battle_phase`/`battle_message`, `battle_moves` (live FIGHT menu while open: cursor + per-slot move/pp/disabled — the `party` PP is a battle-start snapshot, mid-battle drain exists only here), `party` (species/level/hp/status/moves/pp per mon), money/coins, `pc_phase`, `text_speed_delay_frames`, `warp_fade` |
