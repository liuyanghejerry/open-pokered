...
def markdown_report(summary: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 动态像素掩码不同：{totals['dynamic_mask_mismatch']}/{totals['sides']}。",
        f"- 坐标证据：{totals['visible_source_keyframes_match_after_hardware_offset']}/{totals['sides']} 条轨迹的播放器源 OAM 使用硬件坐标；渲染后的 OAM 已按 X−8、Y−16 转为屏幕坐标并纳入上面的实际 OAM 判定。",
        "",
        "## 修复覆盖",
        "",
