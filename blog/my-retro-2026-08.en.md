...
**Different hardware, different programming — the code can't be copied.** The community already has a disassembly of *Pokemon Red*, complete with comments, so I didn't need to mine the ROM for information. But GameBoy-era games were written almost entirely in assembly, tied to a specific CPU architecture. Although AI has largely freed us from reading assembly, this project has 1,923 assembly files and 174K lines of assembly code, and AI can't precisely restore the semantics of every instruction — which means we had to invest heavily in testing.
