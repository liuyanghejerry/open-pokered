// Stage the jrpg-web WASM preview package into dist-electron/wasm-pkg so
...
//
// The pkg is built by `pnpm build:wasm` (wasm-pack) into crates/jrpg-web/pkg.
...
const jrpgSrc = path.resolve(root, '..', '..', 'crates', 'jrpg-web', 'pkg')
// WYSIWYG game preview (pokered-runner-web). Copied into the same staging
// dir as jrpg-web — the /wasm route falls back to it for pokered_runner_web.*
...
for (const [label, src, probe] of [
  ['WASM preview pkg', jrpgSrc, 'jrpg_web.js'],
