...
fn viridian_forest_trainer_engages_on_sight_line() {
    ...
    for _ in 0..120 {
        ...
        if let Some(ref engage) = screen.pending_trainer_engage {
            assert_eq!(engage.trainer_id, "OPP_BUG_CATCHER1");
            // The intro is done: bubble fully counted down (cleared to None
            // on the following frame, same as the script-effect path), and
            // the engage intro consumed.
            assert!(screen
                .pending_emotion_bubble
                .as_ref()
                .map_or(true, |b| b.frames_remaining == 0));
            assert!(screen.trainer_encounter_intro.is_none());
            // DisplayEnemyTrainerTextAndStartBattle: the before-battle text is
            // showing; dismissing it queues the fight.
            assert!(
                screen.pending_dialogue.is_some(),
                "the before-battle text displays after the walk-up"
            );
            ...
            for _ in 0..80 {
                ...
                if screen.pending_trainer_battle.is_some() {
                    assert_eq!(screen.pending_trainer_battle.as_ref().unwrap().trainer_id, "OPP_BUG_CATCHER1");
                    assert!(screen.pending_trainer_engage.is_none());
                    return;
                }
            }
            panic!("dismissed before-battle text did not start the fight");
        }
    }
...
fn talking_to_undefeated_trainer_battles_after_text() {
    ...
    let mut pressed = false;
    for _ in 0..120 {
        if screen.pending_trainer_battle.is_some() || screen.pending_trainer_engage.is_some() {
