...
pub struct PokemonGame {
    ...
    /// update.rs); any frame it doesn't run — sub-screens, the naming screen,
    /// cutscenes, black-screen fades — leaves them stale. The first
    /// overworld frame after such a gap re-baselines them (see
    /// `sync_overworld_input_edges`).
...
impl PokemonGame {
    ...
    /// the naming screen, cutscenes…): the press that drove whatever ran in
    ...
    fn sync_overworld_input_edges(&mut self, input: &InputState<GbButton>) {
        ...
        let action = match self.state.screen {
            ...
            GameScreen::Overworld => {
                if self.overworld.is_naming_screen_active() {
                    ...
                    // screen…): `update_frame` didn't run in between, so its
                    // per-button edge detectors are stale. Re-baseline them —
                    // the press that drove whatever ran in between is still
                    // held and must not re-fire here (e.g. A on START-menu
                    // EXIT instantly talking to a facing NPC).
