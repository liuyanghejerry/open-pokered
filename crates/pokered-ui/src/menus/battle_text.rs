...
///
/// The shared dialog wrapper intentionally treats a single newline as a soft
/// reflow hint.  Gen-1 battle-intro text is different: its control stream
/// explicitly moves `appeared!` to the second tile row, so that boundary must
/// survive even with the narrower proportional frontend font.
pub fn draw_hard_lines<P: Painter>(
    ...
    let wrapped: Vec<String> = text
        .split('\n')
        .flat_map(|line| wrap_lines(line, max_width_px, usize::MAX))
        .collect();
    let start_ty = layout.box_0.text_start_ty.unwrap_or(1);
    let start_tx = layout.box_0.text_start_tx.unwrap_or(0);
    let line_h = layout.box_0.line_height.unwrap_or(2);
