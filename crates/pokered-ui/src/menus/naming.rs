...
/// - pinyin mode: letter rows collapse to step 1 (6..=10) since they select
///   ASCII letters while typing pinyin; the lower half holds the pinyin
///   buffer (row 12) and candidate lines (rows 14/16). CJK glyphs are 10px
///   tall, so lines of Chinese text must stay 2 rows (16px) apart.
const ROW_STEP_ALPHA: u32 = 2;
const ROW_STEP_PINYIN: u32 = 1;
const CASE_ROW_GAP: u32 = 2;
/// Lines 2 rows below each other (pinyin buffer → candidates → overflow).
const INFO_ROW_STEP: u32 = 2;

/// Each candidate occupies a fixed 3-column slot ("[X]" or " X "), so the
/// row is stable while cycling; 6 slots fill the 18-column panel interior.
const CANDIDATE_SLOT_W: u32 = 3;
const CANDIDATES_PER_LINE: usize = 6;
        ...
        if in_pinyin {
            ...
            if !state.pinyin_candidates.is_empty() {
                ...
                for (i, &ch) in state.pinyin_candidates.iter().enumerate() {
                    ...
                    let display = if i == state.candidate_idx {
                        format!("[{ch}]")
                    } else {
                        format!(" {ch} ")
                    };
