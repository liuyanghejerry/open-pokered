...
fn main() {
    ...
    // ── Bare-metal (GBA / thumbv4t): pre-convert gfx PNGs to GB tile data ──
    // Hosted builds decode PNGs at runtime via `image`; bare metal has no
...
fn generate_preconverted_assets(manifest_dir: &str, out_dir: &str) {
    use std::path::PathBuf;

    let gfx_dir = Path::new(manifest_dir).join("../../gfx");
    if !gfx_dir.is_dir() {
        panic!(
            "bare-metal build requires the gfx/ asset tree (run scripts/fetch-gfx.sh first); not found at {}",
            gfx_dir.display()
        );
    }

    let assets_out = Path::new(out_dir).join("gba_assets");
    fs::create_dir_all(&assets_out).expect("create gba_assets dir");

    let mut pngs: Vec<PathBuf> = Vec::new();
    collect_png_files(&gfx_dir, &mut pngs);
    pngs.sort();

    let mut entries: Vec<(String, String, String)> = Vec::new(); // (subdir, stem, include rel path)
    for png in &pngs {
        let stem = png.file_stem().unwrap().to_string_lossy().to_string();
        let subdir = png
            .parent()
            .unwrap()
            .strip_prefix(&gfx_dir)
            .unwrap()
            .to_string_lossy()
            .to_string();
        // Same 1bpp rule as pokered_renderer::resource::AssetCategory::is_1bpp.
        let one_bpp = subdir == "font";
        let data = match convert_png_to_tile_data(png, one_bpp) {
            Ok(d) => d,
            Err(e) => {
                println!(
                    "cargo:warning=gba assets: skipping {}: {}",
                    png.display(),
                    e
                );
                continue;
        ...
        let ext = if one_bpp { "g1bpp" } else { "g2bpp" };
        let file_rel = format!("{}/{}.{}", subdir, stem.replace('/', "_"), ext);
        let dest = assets_out.join(&file_rel);
        fs::create_dir_all(dest.parent().unwrap()).expect("create asset subdir");
        fs::write(&dest, &data).expect("write preconverted asset");
        entries.push((subdir.clone(), stem.clone(), format!("gba_assets/{file_rel}")));
    }

    let registry = Path::new(out_dir).join("gba_assets.rs");
    let mut m = BufWriter::new(File::create(&registry).unwrap());
    writeln!(m, "// @generated by pokered-renderer build.rs — do not edit.").unwrap();
    writeln!(m).unwrap();
    writeln!(
        m,
        "pub static PRECONVERTED_ASSETS: &[(&'static str, &'static str, &[u8])] = &["
    )
    .unwrap();
    for (subdir, stem, rel) in &entries {
        writeln!(
            m,
    ...
    writeln!(
        m,
        "pub fn get_preconverted_asset(subdir: &str, name: &str) -> Option<&'static [u8]> {{"
    )
    .unwrap();
    writeln!(
        m,
        "    PRECONVERTED_ASSETS.iter().find(|(s, n, _)| *s == subdir && *n == name).map(|(_, _, b)| *b)"
    )
    .unwrap();
    writeln!(m, "}}").unwrap();
    m.flush().unwrap();
...
fn collect_png_files(dir: &Path, out: &mut Vec<std::path::PathBuf>) {
...
fn convert_png_to_tile_data(png: &Path, one_bpp: bool) -> Result<Vec<u8>, String> {
    use image::GenericImageView;
    let img = image::open(png).map_err(|e| e.to_string())?;
    let (w, h) = img.dimensions();
    if w % 8 != 0 || h % 8 != 0 {
        return Err(format!("dimensions {w}x{h} not multiples of 8"));
    }
    let gray = img.to_luma8();
    let (tiles_x, tiles_y) = ((w / 8) as usize, (h / 8) as usize);

    // Same quantization as dotzuki_renderer::resource::grayscale_to_color_index.
    fn color_index(v: u8) -> u8 {
        match v {
            213..=255 => 0,
            128..=212 => 1,
            43..=127 => 2,
    ...
    let mut data = Vec::with_capacity(tiles_x * tiles_y * if one_bpp { 8 } else { 16 });
    for ty in 0..tiles_y {
        for tx in 0..tiles_x {
            for row in 0..8usize {
                let mut lo: u8 = 0;
                ...
                for col in 0..8usize {
                    ...
                    if one_bpp {
                        // 1bpp: bit 1 = black, bit 0 = white.
                        if px < 128 {
                            lo |= 1 << (7 - col);
                        }
                    } else {
                        let color = color_index(px);
                        let bit = 7 - col;
                        lo |= (color & 1) << bit;
                        hi |= ((color >> 1) & 1) << bit;
                    }
