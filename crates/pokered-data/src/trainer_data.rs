...
/// ROM-resident trainer tables (generated by build.rs from trainers/*.json):
/// `TRAINER_CLASS_ORDER` lists the classes in `TrainerClass` numeric order
/// (Nobody excluded) and `trainer_parties(class)` returns that class's
/// parties as static slices — zero heap involvement.
include!(concat!(env!("OUT_DIR"), "/trainer_data_gen.rs"));

/// Get all trainer class data. Materializes the owned `Vec` form (editor /
/// benches / JSON round-trips); battle entry points should prefer
/// [`get_trainer_party_mons`], which reads the statics directly.
pub fn trainer_data() -> Vec<TrainerClassData> {
    TRAINER_CLASS_ORDER
        .iter()
        .map(|&class| TrainerClassData {
            class,
            parties: trainer_parties(class)
                .iter()
                .map(|mons| TrainerParty {
                    pokemon: mons.to_vec(),
                })
                .collect(),
        })
...
pub fn get_trainer_party_mons(class: TrainerClass, party_index: usize) -> Option<&'static [TrainerMon]> {
    // Editor-injected runtime override shadows the baseline.
    if let Some(ov) = crate::runtime_overrides::trainer_override(class) {
        return ov.parties.get(party_index).map(|p| p.pokemon.as_slice());
    }
    trainer_parties(class).get(party_index).copied()
}
