...
两条完整录像均来自单次 NEW GAME 连续运行。不同失败尝试没有拼接进成功过程。脚本一侧标注“适配与恢复修复”：菜单兼容、森林败退后的训练时机、西尔佛及后续道馆重试、战斗用药及联盟重试均为人工改动，详见主文档与[补丁](full-run/script-driver-adaptation.patch)。Jev 不调用 m01–m49 既有路线。
