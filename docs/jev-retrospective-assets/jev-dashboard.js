...
(function () {
  ...
  function markup(){return `<div class="jd-head"><span class="jd-eyebrow">JEV 决策与执行记录</span><span class="jd-badges"></span></div>
  ...
  function render(root,second){
    ...
    const put=(selector,text)=>{const e=root.querySelector(selector);if(e&&e.textContent!==text)e.textContent=text;};
    ...
    const party=(state.party||[]).map(m=>`<div class="jd-mon"><div class="jd-mon-label"><b>${esc(m.name)} <span>Lv.${m.level}</span></b><span>${m.hp}/${m.max_hp} HP</span></div><div class="jd-hp"><i style="width:${Math.max(0,Math.min(100,m.hp/m.max_hp*100))}%;background:${m.hp/m.max_hp<.25?'#a3472b':'#237451'}"></i></div></div>`).join('')||'<div class="jd-empty">尚未领取初始宝可梦</div>';
    const partyRoot=root.querySelector('.jd-party');if(partyRoot.innerHTML!==party)partyRoot.innerHTML=party;
    put('.jd-goal',strategy.title||'等待首次策略选择');
    put('.jd-prob-label',strategy.model_choice?'分布非成功率 · 本次由代码从非拒选项中选择':'候选前三项 · 选择分布，不是成功率');
    const candidates=(strategy.candidates||[]).map(c=>`<div class="jd-option ${c.selected?'jd-selected':''}"><div class="jd-option-label"><span>${c.selected?'✓ ':''}${esc(c.label)}</span><b>${Math.round(c.p*100)}%</b></div><div class="jd-prob"><i style="width:${c.p*100}%"></i></div></div>`).join('')||'<div class="jd-empty">尚无模型判断</div>';
    const candidateRoot=root.querySelector('.jd-candidates');if(candidateRoot.innerHTML!==candidates)candidateRoot.innerHTML=candidates;
  ...
  function loadRaw(){
    ...
    if(!rawLoading)rawLoading=new Promise((resolve,reject)=>{const tag=document.createElement('script');const base=document.querySelector('script[src$="jev-dashboard-data.js"]');tag.src=new URL('jev-inputs-data.js',base.src).href;tag.onload=()=>resolve(window.JEV_INPUTS);tag.onerror=()=>{rawLoading=null;tag.remove();reject(new Error('输入文件加载失败，请保留 jev-inputs-data.js。'));};document.head.appendChild(tag);});
    return rawLoading;
  }
  function inputMarkup(interactive=true){return ['strategy','action'].map(layer=>`<section class="jd-input-card" data-layer="${layer}"><div class="jd-input-title"></div><div class="jd-input-meta"></div><dl class="jd-input-rows"></dl>${interactive?'<details class="jd-raw"><summary>展开当次完整输入、全部候选与输出</summary><p>应用层调用原文：state 是状态，question 是指令与全部候选，answer 是模型输出；controller_selection 如存在，表示代码二次选择。此处不包含模型隐藏推理。</p><pre></pre></details>':''}</section>`).join('');}
  async function showRaw(card){const detail=card.querySelector('.jd-raw');if(!detail?.open)return;const id=card.dataset.inputId;const pre=detail.querySelector('pre');if(!id){pre.textContent='该层尚未发生模型调用。';return;}try{const all=await loadRaw();if(card.dataset.inputId===id)pre.textContent=JSON.stringify(all[id],null,2);}catch(error){pre.textContent=error.message;}}
  function renderInputs(root,second){if(!root||!window.JEV_DASHBOARD)return;for(const card of root.querySelectorAll('.jd-input-card')){
    ...
    if(card.dataset.inputId!==id){card.dataset.inputId=id;
      card.querySelector('.jd-input-title').textContent=`${layer==='strategy'?'策略层':'动作层'} · 当次输入${entry?' #'+entry.id+' / '+entry.kind:''}`;
      card.querySelector('.jd-input-rows').innerHTML=entry?entry.rows.map(([k,v])=>`<dt>${esc(k)}</dt><dd title="${esc(v)}">${esc(v)}</dd>`).join(''):'<dd>该层尚未发生模型调用</dd>';
      showRaw(card);
    }
    card.querySelector('.jd-input-meta').textContent=entry?`原片快照 ${clock(entry.source_s)} · 距画面 ${Math.max(0,Math.floor(second-entry.source_s))} 秒 · ${entry.non_abstain_candidates} 个候选`:'只展示实际传入信息，不以当前状态补齐';
  ...
  function noteAt(second){const notes=window.JEV_DASHBOARD?.notes||[];return [...notes].reverse().find(n=>n.source_s<=second+0.00001&&second<n.source_s+240)||null;}
