...
(function () {
  ...
  const labels = {
    ...
    '脚本（无模型参与）': 'Scripted run · no model involvement',
    '左侧录像 · 纯脚本运行': 'Left recording · scripted run',
    '从 NEW GAME 到名人堂完全由预写脚本驱动，全程没有任何模型参与，作为右侧 Jev 录像的对照基线。': 'From NEW GAME to the Hall of Fame, this run is driven entirely by a prewritten script with no model involvement, serving as the baseline for the Jev recording on the right.',
    '因此本栏没有模型输入卡片——右栏的「当次输入」快照只来自 Jev 录像。': 'This column therefore has no model input cards — the “latest input” snapshots in the right column come only from the Jev recording.',
    '两路使用相同引擎与 seed 42，按共同剧情节点分章对照。': 'Both runs use the same engine build and seed 42, divided into chapters by shared story milestones.',
    '策略层 / 动作层 · 当次输入快照 — 仅对应右侧 Jev 录像': 'Strategy / action layer input snapshots — from the right-side Jev recording only',
