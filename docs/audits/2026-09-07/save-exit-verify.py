...
OUT=Path('/tmp/pokered-completion-20260907-save-exit-check')
...
shutil.copy2(ROOT/'docs/audits/2026-09-07/m51-saffron-fly.sav',OUT/'active.sav')
shutil.copy2(ROOT/'docs/audits/2026-09-07/m51-saffron-fly.script_flags.json',OUT/'bin/pokered.script_flags.json')
...
try:
 ...
 g.nav_to(11,15,'SilphCo1F');g.cutscene();g.step(60)
 checkpoint('indoor-saved-new-context')
 shutil.copy2(g.run_dir/'game.log',OUT/'first-game.log');g.close()
 g=AuditGame(save_path=OUT/'active.sav');orig=g.d.cmd;g.d.cmd=cmd
 g.cutscene=types.MethodType(advance_cutscene,g)
 resume_reentry(g);g.cutscene();g.step(60)
 snap('indoor-continued')
 assert g.st()['map_name']=='SilphCo1F',g.st()
 g.nav_warp(11,17,'SilphCo1F','SaffronCity',approach='down')
 snap('indoor-continued-exited')
 assert g.st()['map_name']=='SaffronCity',g.st()
 print('SAVE_RESTART_EXIT_PASSED',g.pos(),flush=True)
finally:
