...
## 修复验收纠正与补修（2026-09-08）

复验发现此前完成总结不够准确：红莲答案/门号/训练家胜利开门、静态遭遇 SRAM 映射、对象显隐回写、冠军之路落石、TUI PP 道具与运行时地图等仍有漏修；翻译脚本曾检查 0 个场景，不能算通过。上述缺口已补修，并合入远端 master `0c32b9d`，处理塔 7F 加载合并及新游戏空队伍误全灭回归。

逐项发现、测试名称、日志与当前验收结果见 [acceptance.md](acceptance.md)。重新归档 app/TUI 各 18 组 master 同状态前后截图；验证覆盖范围与原始冠军通关记录明确区分。B03 未独立复现及 B08 部分旧档无法恢复历史选择的限制继续保留，不宣称无条件全部修复。
