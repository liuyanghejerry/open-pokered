...
OUT=Path('/tmp/pokered-completion-20260907-saffron-guard-repro')
...
shutil.copy2(ROOT/'docs/audits/2026-09-07/m67-saffron-gym-blocked.sav',OUT/'active.sav')
shutil.copy2(ROOT/'docs/audits/2026-09-07/m67-saffron-gym-blocked.script_flags.json',OUT/'bin/pokered.script_flags.json')
...
try:
 ...
 assert g.d.cmd(cmd='get_flags')['data'].get('EVENT_BEAT_SILPH_CO_GIOVANNI')
 assert g.d.cmd(cmd='get_npcs')['data'][2]['visible']
 g.face('left');g.tap('a',20);snap('gym-guard-after-continue');g.cutscene();g.step(40)
 g.d.drive(['left']*24,frames=24);g.cutscene();g.step(40)
 s=snap('gym-guard-blocks-after-continue');assert s['player_x']==35,s
 print('GYM_GUARD_STILL_BLOCKS_AFTER_LIBERATION_REPRODUCED',flush=True)
finally:
