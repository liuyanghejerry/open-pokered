...
This branch is paired with dotzuki's `feat/shared-mobile-host` branch. Until that engine change is released, use the explicit local Cargo patch generated below; the existing `v0.6.0` tag does not contain the new crate/features. Do not mix engine crates from different revisions. A release must replace the old tag consistently and regenerate Cargo.lock without local patches.
...
The desktop frontend retains its default GPU/device-audio behavior. The mobile build disables desktop features and uses embedded graphics. Android/iOS shells have not been migrated in this change.

## Validation on 2026-09-12

Both repositories use `feat/shared-mobile-host`; the engine baseline is `341ca857`, and the game baseline is `746f7dd`.
...
The emulator captures demonstrate the new platform; there is no baseline Harmony build of this game. Full playthrough, physical-device audio/latency, suspend/resume stress and on-device save/relaunch testing remain release validation work. Android/iOS migration is a separate follow-up.
